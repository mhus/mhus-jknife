...
*/
package de.mhus.jknife.jllmrequest;

import de.mhus.jknife.jllm.shared.LlmConfig;
import de.mhus.jknife.jllm.shared.LlmConfigLoader;
import de.mhus.jknife.jllm.shared.Llms;
...
/**
 * jllmrequest - send a prompt to an llm and print the response (text in, text out).
 */
@Command(name = "jllmrequest", mixinStandardHelpOptions = true, version = "jllmrequest "
        + JllmRequestCmd.VERSION, description = "Send a prompt to an llm and print the response (text in, text out). Providers: openai, ollama. Exit code 0 on success, 2 on error.")
public class JllmRequestCmd implements Callable<Integer> {
    ...
    public static void main(String... args) {
        int exitCode = new CommandLine(new JllmRequestCmd()).execute(args);
