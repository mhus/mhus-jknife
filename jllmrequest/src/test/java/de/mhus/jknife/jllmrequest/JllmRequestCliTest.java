...
class JllmRequestCliTest {
    ...
    private Result run(String... args) {
        ...
        try {
            ...
            int exit = new CommandLine(new JllmRequestCmd()).execute(args);
    ...
    void requestOpenAi() throws Exception {
        ...
        var r = run("--llm", config, "say hello");
    ...
    void requestOpenAiWithSystemPrompt() throws Exception {
        ...
        var r = run("--llm", config, "--system", "You are a pirate.", "say hello");
        assertThat(r.exitCode()).isZero();
    ...
    void requestWithConfigFileAndInlineOverride() throws Exception {
        ...
        var r = run("--llm-config", file.toString(), "--llm", "{model: gpt-4o-mini}", "say hello");
        assertThat(r.exitCode()).isZero();
    ...
    void missingConfigAndEmptyPrompt() throws Exception {
        assertThat(run("say hello").exitCode()).isEqualTo(2);

        var server = startOllama();
        var config = "{provider: ollama, model: llama3.1, baseUrl: '" + server.baseUrl() + "'}";
        assertThat(run("--llm", config, "").exitCode()).isEqualTo(2);
        assertThat(run("--llm", "{invalid", "hi").exitCode()).isEqualTo(2);
    }
}
