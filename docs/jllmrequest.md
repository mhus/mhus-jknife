...
LLM request tool — text in, text out. Based on
[langchain4j](https://docs.langchain4j.dev/), providers: **openai**, **ollama**.

## Usage

```
jllmrequest [options] [prompt]    # prompt as argument or from stdin
```

## Examples

```shell
jllmrequest --llm '{provider: ollama, model: llama3.1}' "explain quantum computing"

echo "translate to german: hello" | jllmrequest --llm-config llm.yaml

# system prompt
jllmrequest --llm-config llm.yaml --system "You are a pirate." "say hello"

# openai (api key from env OPENAI_API_KEY or config)
jllmrequest --llm '{provider: openai, model: gpt-4o-mini}' "hi"
```
...
```shell
jllmrequest --llm-config llm.yaml --llm '{model: gpt-4o}' "hi"
```
...
`jllmrequest` is the first tool of the llm family. All llm tools share the
`jllm-shared` module (config loading, langchain4j model factories, graalvm metadata),
so new llm commands build on the same configuration handling.

## Install

```shell
brew tap mhus/jknife
brew install jllmrequest
```
